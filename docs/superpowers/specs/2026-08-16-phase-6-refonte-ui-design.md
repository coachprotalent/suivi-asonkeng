...
| D116 | **Aucun thème sombre dans cette phase.** Les jetons sont structurés pour l'accueillir plus tard sans retoucher un seul composant. **Amendement du 2026-08-16 : c'est un RETRAIT, pas une abstention.** Un thème sombre hérité de `create-next-app` est **actif en production et déjà cassé** — `globals.css:15-20` bascule `--background` à `#0a0a0a` sous `prefers-color-scheme: dark`, tandis que les écrans peignent leurs fonds en classes Tailwind claires. La phase le **supprime** | Un thème sombre double la surface de preuve visuelle de **tous** les écrans, pour un besoin que personne n'a exprimé. Le structurer sans le livrer coûte zéro ; le livrer coûte la phase entière une seconde fois. La décision est écrite ici pour que « on l'ajoutera plus tard » soit une phrase vérifiable et non une intention. **L'amendement change la nature du travail** : je croyais décider de ne rien ajouter ; le code montre qu'il faut retirer quelque chose qui nuit déjà. Tout utilisateur dont le système est en thème sombre voit aujourd'hui une application partiellement illisible, et personne ne l'a signalé — ce qui dit surtout que personne ne l'a regardée dans cet état |
| D128 | **Les deux polices `Geist` et `Geist_Mono`, chargées puis neutralisées, sont retirées.** La pile système devient ce que le code fait déjà, au lieu de ce qu'il paie sans le faire | Mesuré : `layout.tsx:2,6-14` charge les deux familles par `next/font/google`, `globals.css:8-13` les mappe sur `--font-sans` et `--font-mono` — et `globals.css:25` écrase le tout par `font-family: Arial, Helvetica, sans-serif` sur le `body`. Les polices sont **téléchargées, mises en cache, servies, et jamais rendues**, hors trois `<code>`. Le §4.2 de ce document affirmait « aucune police n'est chargée » : c'était **faux**, et c'est le genre d'affirmation que ce projet appelle une carte fausse. Le retrait rend vraie une phrase qui ne l'était pas, et supprime un coût réel au premier rendu de chaque page |
| D129 | **Une exception unique et bornée à D119 est déclarée d'avance** : le remplacement des `window.confirm` (D124) rend **inertes** 22 gestionnaires `page.once('dialog', …)` répartis sur **11 fichiers de preuve**, dont **une des dix preuves de production**. Ces gestionnaires sont adaptés ; **aucune assertion n'est touchée**, et les totaux — 128 et 10 — sont vérifiés par comptage avant et après. **Amendement du 2026-08-16 : la borne réelle est plus large d'un cran que celle que j'avais écrite.** Le `Dialogue` ajoute un cycle de rendu, et ce cycle a effacé la valeur qu'une preuve de production **forçait dans le DOM** avant de confirmer. Ce n'est pas un gestionnaire de dialogue : c'est une **préparation** de test que le changement rend caduque. La borne se lit donc : **toute adaptation de harnais rendue nécessaire par le passage au `Dialogue`, sans qu'aucune assertion ne change** | D119 et D124 sont **incompatibles tels qu'écrits**, et je ne l'avais pas vu. Un gestionnaire de dialogue natif qui n'a plus de dialogue à intercepter ne rougit pas : il **expire**, loin de sa cause, et le diagnostic naturel serait alors « la refonte a cassé l'écran ». Déclarer l'exception d'avance, la borner à une seule tâche et interdire toute retouche d'assertion est la seule façon de garder D119 utilisable comme filet : une règle absolue qu'on enfreint en silence à la première difficulté ne protège plus rien. **Toute autre preuve qui rougit reste un signal**, sans exception |
| D117 | **Aucun texte affiché n'est modifié.** Les libellés, messages de refus, titres et états gardent leur formulation **à l'octet près**, sauf ceux qu'un tableau du plan liste explicitement, avec leur justification. **Declaration du 2026-08-16 — un texte affiche nouveau, et un seul :** l'etat d'une fiche est desormais rendu sur `/membres/[id]` par une etiquette portant *Actif*, *En attente* ou *Archive*. Il n'y etait **pas affiche du tout** auparavant : ce n'est donc pas une reformulation mais un **ajout**, et D126 l'exige des lors qu'un etat est montre — pastille **et** libelle, jamais l'un sans l'autre. Les deux premiers libelles sont **repris du vocabulaire deja employe** ailleurs dans l'application (`comptes/ligne-compte.tsx`, `demandes/ligne-demande-personnelle.tsx`), donc rien n'est invente. **Vigilance pour la suite :** une preuve assert deja `getByText('En attente')` ; ajouter le meme libelle a une page qui en porte deja un rendrait l'assertion **ambigue** en mode strict, et elle rougirait pour une raison qui n'aurait rien a voir avec le defaut cherche | La suite `test:e2e:prod` assert des textes affichés ; les suites `test:e2e` aussi. Un changement de formulation non déclaré ferait rougir des preuves **pour la mauvaise raison**, et l'ajustement du test serait alors le geste naturel et faux. Cette phase change la **présentation** ; le vocabulaire de l'application a été arbitré ailleurs |
...
| D124 | **Les quinze `window.confirm()` sont remplacés par un `Dialogue` unique**, fondé sur `<dialog>` natif : focus piégé, `Échap` ferme, retour du focus au déclencheur | `window.confirm()` n'est pas stylable, bloque le fil d'exécution, se présente hors de la page, et sur mobile s'affiche comme une alerte système que rien ne rattache à l'application. Le remplacer est le seul geste de cette phase qui change un **comportement** perceptible — c'est pourquoi il est nommé ici et non traité comme un détail de présentation. **Amendement du 2026-08-16 : « le seul » est devenu faux, et il faut le dire.** La migration de `/connexion` a rendu le champ de mot de passe **contrôlé** ; il **survit** donc désormais à un refus, là où React le vidait. C'est un second changement de comportement perceptible, voulu, conforme au brief et déclaré dans le code — mais il n'était ni au rapport ni au registre, et cette phrase-ci affirmait qu'il n'existait pas |
...
| D126 | **Un état est rendu par une pastille de couleur suivie du libellé**, jamais par le libellé seul, jamais par une pastille seule. **Amendement du 2026-08-16 : la liste d'états que portait cette décision était inventée.** Le dépôt porte **quatre vocabulaires d'état distincts** — l'état d'une fiche (`membre.ts:2`), celui d'une demande (`demandes.ts:50`), celui d'une séance AEL (`ael.ts:4`), et l'actif/inactif d'un compte —, plus les **lignes de catalogue éditables** de `public.statuts`, qui ne sont **pas** des états. `EtatBadge` sert les quatre vocabulaires ; il ne sert **pas** les statuts | La direction « Filiation » ne colore pas le fond de l'étiquette, contrairement à ce qu'aurait fait « Atelier ». La couleur reste donc un **second canal**, jamais le seul : la pastille aide au repérage, le libellé porte le sens. Une couleur seule serait invisible à qui ne la distingue pas, et un libellé seul perdrait le bénéfice du repérage qui a justifié D107. **Sur l'amendement :** j'avais écrit « Repenti, Baptisé, Affermi » en confondant les statuts du catalogue avec les états de la machine, et **`Affermi` n'existe nulle part dans le dépôt** — je l'avais pris dans mes propres maquettes, où il était un exemple. Un composant construit sur cette liste aurait mêlé deux notions que le modèle sépare depuis la phase 1b : un statut s'attribue et se retire avec un journal, un état se transitionne sous déclencheurs. **Conséquence directe : `EtatBadge` a ZÉRO antécédent à extraire, et non deux** — les deux « pastilles » que j'avais comptées (`membres/[id]/page.tsx:213` et `:255`) sont des puces sans couleur. Il change donc de catégorie et rejoint `Carte` et `Dialogue` : son risque est le **défaut**, pas la divergence |
...
**Correction du 2026-08-16 — cette section affirmait « aucune police n'est chargée » : c'était
faux.** Le dépôt charge `Geist` et `Geist_Mono` par `next/font/google` (`layout.tsx:2,6-14`), les
mappe sur `--font-sans` et `--font-mono` (`globals.css:8-13`), **puis les écrase** par
`font-family: Arial, Helvetica, sans-serif` sur le `body` (`globals.css:25`). Elles sont donc
téléchargées et servies sans être rendues, hors trois `<code>`. **D128 les retire.**

Aucune police n'est chargée **à l'issue de la phase**, et c'est un choix : une police
distante introduit un octet bloquant sur le premier rendu de chaque page pour un bénéfice
esthétique. La pile système est **choisie**, pas subie : `"Segoe UI Variable Text", "Segoe UI",
